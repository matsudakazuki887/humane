...
void Main()
{
	...
	while (System::Update())
	{
		...
		for (i = 1; i <= 5; i++)
		{
			Line{ calendar_x1, calendar_y2 / 5 * i + calendar_y1 , calendar_x2 + calendar_x1 ,calendar_y2 / 5 * i + calendar_y1 }.draw(0.5, Palette::Silver); //横の線
		}
		if (uru(y, m))month[2]++;
		for (i = 1; i <= month[m]; i++)
		{
			fontBold(i).draw(calendar_x2 / 7 * ((i+h - 1) % 7) + calendar_x1+4, calendar_y2/5 * j + calendar_y1+2, Palette::Black); //日付
			if ((i+h) % 7 == 0)j++;
		}
		j = 0;
		if (uru(y, m))month[2]--;
		for (i = 1; i <=7; i++)
		{
			if(i==1) fontBold(text[i - 1]).draw(calendar_x2 / 7 * i + calendar_x1 - 57, calendar_y1 - 20, Palette::Red);
